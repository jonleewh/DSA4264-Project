...
DEFAULT_STEM1_MODULES = PROJECT_ROOT / "data" / "cleaned_data" / "cleaned_module_rows_STEM.jsonl"
DEFAULT_TEST_MODULE_INPUT = TEST_DIR / "module_descriptions_test_with_skills_independent_STEM.jsonl"
DEFAULT_TEST_JOB_INPUT = TEST_DIR / "job_ssoc345_with_skills_from_original_STEM.jsonl"
DEFAULT_TEST_MODULE_OUTPUT = TEST_DIR / "module_skills_canonical_stem.jsonl"
DEFAULT_TEST_JOB_OUTPUT = TEST_DIR / "job_skills_canonical_stem.jsonl"
DEFAULT_FULL_MODULE_INPUT = FULL_DIR / "module_descriptions_STEM_with_skills_independent.jsonl"
DEFAULT_FULL_JOB_INPUT = FULL_DIR / "job_ssoc345_with_skills_from_original_STEM.jsonl"
DEFAULT_FULL_MODULE_OUTPUT = FULL_DIR / "module_skills_canonical_stem.jsonl"
DEFAULT_FULL_JOB_OUTPUT = FULL_DIR / "job_skills_canonical_stem.jsonl"
...
class CanonicalSkillMapper:
    ...
    def map_phrase(self, phrase: str) -> dict:
        ...
        if cached is not None:
            return {**cached, "raw_phrase": phrase, "normalized_phrase": normalized}
...
def parse_args():
    parser = argparse.ArgumentParser(description="Map raw STEM phrases into a canonical skill framework.")
    parser.add_argument("--framework", type=Path, default=DEFAULT_FRAMEWORK)
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL)
    parser.add_argument("--threshold", type=float, default=0.72)
    parser.add_argument("--full-dataset", action="store_true")
    parser.add_argument("--phrases", type=str, default="")
    ...
    parser.add_argument("--map-stem-pipeline", action="store_true")
    parser.add_argument("--module-input-jsonl", type=Path, default=None)
    parser.add_argument("--module-output-jsonl", type=Path, default=None)
    parser.add_argument("--job-input-jsonl", type=Path, default=None)
    parser.add_argument("--job-output-jsonl", type=Path, default=None)
    return parser.parse_args()


def resolve_pipeline_paths(full_dataset: bool, args):
    if full_dataset:
        module_input = args.module_input_jsonl or DEFAULT_FULL_MODULE_INPUT
        module_output = args.module_output_jsonl or DEFAULT_FULL_MODULE_OUTPUT
        job_input = args.job_input_jsonl or DEFAULT_FULL_JOB_INPUT
        job_output = args.job_output_jsonl or DEFAULT_FULL_JOB_OUTPUT
    else:
        module_input = args.module_input_jsonl or DEFAULT_TEST_MODULE_INPUT
        module_output = args.module_output_jsonl or DEFAULT_TEST_MODULE_OUTPUT
        job_input = args.job_input_jsonl or DEFAULT_TEST_JOB_INPUT
        job_output = args.job_output_jsonl or DEFAULT_TEST_JOB_OUTPUT
    return module_input, module_output, job_input, job_output
...
def map_jsonl_file(
    ...
    for row in rows:
        ...
        row[output_field] = sorted({item["canonical_skill"] for item in mapped if item["canonical_skill"]})
...
def main():
    ...
    if run_pipeline_mode:
        module_input, module_output, job_input, job_output = resolve_pipeline_paths(args.full_dataset, args)
        prerequisite_paths = [
            ("stem_1", DEFAULT_STEM1_MODULES),
            ("stem_3 module skills", module_input),
            ("stem_4 job skills", job_input),
        ]
        missing = [(label, path) for label, path in prerequisite_paths if not path.exists()]
        if missing:
            lines = [f"- {label}: {path}" for label, path in missing]
            raise FileNotFoundError("Missing required stem pipeline files:\n" + "\n".join(lines))

        module_count = map_jsonl_file(
            mapper=mapper,
            input_jsonl=module_input,
            output_jsonl=module_output,
            input_field=args.input_field,
            output_field=args.output_field,
            batch_size=args.batch_size,
        )
        job_count = map_jsonl_file(
            mapper=mapper,
            input_jsonl=job_input,
            output_jsonl=job_output,
            input_field=args.input_field,
            output_field=args.output_field,
            batch_size=args.batch_size,
        )
        ...
        print(f"Saved module output: {module_output}")
        print(f"Mapped job rows: {job_count}")
        print(f"Saved job output: {job_output}")
        print(f"Framework used: {args.framework}")
        return

    raise SystemExit(
        "Provide either --phrases, or both --input-jsonl/--output-jsonl, or use the default stem pipeline modes."
    )
