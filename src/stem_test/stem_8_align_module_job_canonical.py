...
DEFAULT_TEST_MODULE = TEST_DIR / "module_skills_canonical_stem.jsonl"
DEFAULT_TEST_JOB = TEST_DIR / "job_skills_canonical_stem.jsonl"
DEFAULT_TEST_OUTPUT = TEST_DIR / "module_job_alignment_STEM.json"
DEFAULT_FULL_MODULE = FULL_DIR / "module_skills_canonical_stem.jsonl"
DEFAULT_FULL_JOB = FULL_DIR / "job_skills_canonical_stem.jsonl"
DEFAULT_FULL_OUTPUT = FULL_DIR / "module_job_alignment_STEM.json"
...
def resolve_paths(full_dataset: bool, module: Path | None, job: Path | None, output: Path | None):
    default_module = DEFAULT_FULL_MODULE if full_dataset else DEFAULT_TEST_MODULE
    default_job = DEFAULT_FULL_JOB if full_dataset else DEFAULT_TEST_JOB
    default_output = DEFAULT_FULL_OUTPUT if full_dataset else DEFAULT_TEST_OUTPUT
    return module or default_module, job or default_job, output or default_output


def main():
    parser = argparse.ArgumentParser(description="Align STEM modules and jobs in canonical skill space.")
    ...
    module_path, job_path, output_path = resolve_paths(args.full_dataset, args.module, args.job, args.output)

    module_rows = load_jsonl(module_path)
    job_rows = load_jsonl(job_path)
    ...
    for row in module_rows:
        ...
        if matches and matches[0]["alignment_score"] > 0:
            top1_positive += 1
    ...
    summary = {
        ...
        "job_group_count": len(profiles),
        "top1_positive_modules": top1_positive,
        "top1_overlap_modules": top1_overlap,
        "top1_positive_rate": round(top1_positive / module_count, 4) if module_count else 0.0,
        "top1_overlap_rate": round(top1_overlap / module_count, 4) if module_count else 0.0,
        "average_top1_score": round(
            sum(r["top_matches"][0]["alignment_score"] for r in non_empty_results if r["top_matches"])
            / module_count,
            4,
        ) if module_count else 0.0,
        "average_top1_good_job_pct": round(
            sum(r["top_matches"][0]["good_job_pct"] for r in non_empty_results if r["top_matches"])
            / module_count,
            4,
        ) if module_count else 0.0,
        "average_top1_quality_weighted_alignment": round(
            sum(r["top_matches"][0]["quality_weighted_alignment_score"] for r in non_empty_results if r["top_matches"])
            / module_count,
            4,
        ) if module_count else 0.0,
        "average_top3_weighted_good_job_pct": round(
            sum(r["top3_weighted_good_job_pct"] for r in non_empty_results if r["top_matches"])
            / module_count,
            4,
        ) if module_count else 0.0,
    }
    ...
    print(f"Saved canonical alignment: {output_path}")
